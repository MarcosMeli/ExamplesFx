...
namespace ExamplesFx.Tests
{
    ...
    public class ParseTest
    {
        ...
        public void GenerateFileHelpersDoc()
        {
            ...
            var examplesContent = new Dictionary<Tuple<string, ExampleCode>, string>();

            ExampleCode previousExample = null;
...
breadcrumblink2: examples/#" + ex.CategoryFileName + @"
breadcrumbtext2: " + ex.Category + @"
---

<div style='margin-top: -10px; opacity: 0.7;'>
{% if '[[PreviousExampleName]]' != '' %}
  <div class='waves-effect waves-dark' style='float: left;'>
    <a title='Previous Example' href='[[PreviousExampleLink]]' style='color: #EE6930;'><i class='fa fa-arrow-left'></i> Previous: [[PreviousExampleName]]</a>
  </div>
{% endif %}

{% if '[[NextExampleName]]' != '' %}
  <div class='waves-effect waves-dark' style='float: right;'>
    <a title='Next Example' href='[[NextExampleLink]]' style='color: #EE6930;'>Next: [[NextExampleName]] <i class='fa fa-arrow-right'></i></a>
  </div>
{% endif %}
</div>
...
<div class='waves-effect waves-dark' style='padding: 6px; text-align: right; float: right;clear: both; margin-bottom: -30px; margin-top:-70px;'>
    <span class=''>Click <a title='Autogenerated example click here if you want to edit and submit changes' href='{{ page.editlink }}'><i class='mdi-editor-mode-edit'></i> button</a> to edit</span>
</div>

<div style='padding-top: 5px;clear: both;'>
<h5>" + ex.Category + ": " + ex.Name + @"</h5>
<div class='indent'>
  <h6 style='font-size: 120%;'>" + ex.Description + @"</h6>
  <div class='indent'>
");
                ...
                if (previousExample != null)
                    previousExample.NextExample = ex;

                ex.PreviousExample = previousExample;

                examplesContent.Add(Tuple.Create(examplePath, ex), html.ToString());
            ...
            foreach (var exampleEntry in examplesContent)
            {
                var previousName = "";
                var previousLink = "";
                var nextName = "";
                ...
                var example = exampleEntry.Key.Item2;

                if (example.PreviousExample != null)
                {
                    previousName = example.PreviousExample.Name;
                    previousLink = example.PreviousExample.Url;
                }

                if (example.NextExample != null)
                {
                    nextName = example.NextExample.Name;
                    nextLink = example.NextExample.Url;
                }

                var contents = exampleEntry.Value
                    .Replace("[[PreviousExampleName]]", previousName)
                    .Replace("[[PreviousExampleLink]]", previousLink)
                    .Replace("[[NextExampleName]]", nextName)
                    .Replace("[[NextExampleLink]]", nextLink);

                File.WriteAllText(exampleEntry.Key.Item1, contents);
            }
        ...
        private ExampleCode CreateExample(Document doc, Assembly assembly)
        {
            ...
            res.Url = "/example/" + (res.CategoryFileName.Length > 0 ? res.CategoryFileName + "/" : "") +
...
");
            ...
            foreach (var category in res.GroupBy(x => x.Category))
            {
                ...
                currentRow.AppendLine("<h5><a name='" + category.Key.Replace(" ", "").Replace("/", "-") + "'></a>" +
                                      category.Key + "</h5>");
